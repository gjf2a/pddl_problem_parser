...
pub struct PddlProblem {
    ...
    pub f64_state: BTreeMap<Predicate,f64>,
...
impl PddlProblem {
    pub fn new() -> Self {
        PddlProblem {name: String::new(), domain: String::new(),
            ...
            f64_state: BTreeMap::new(), goals: BTreeSet::new(), metric: None}
    }
}
...
impl PddlParser {
    pub fn parse(pddl: &str) -> PddlProblem {
    ...
    fn snag_predicate(&mut self) -> Predicate {
        self.check("(");
    ...
    fn snag(&mut self) -> String {
        let result = String::from(self.token());
        self.advance();
        result
    }
    ...
    fn lookahead(&self, distance: usize) -> &str {
        self.tokens[self.i + distance].as_str()
    }

    fn check(&mut self, target_token: &str) {
        assert_eq!(self.token(), target_token);
        self.advance();
    }
    ...
    fn init(&mut self) {
        ...
        while !self.at_close() {
            if self.lookahead(1) == "=" {
                ...
                let value = self.token().parse::<f64>().unwrap();
                self.advance();
                self.check(")");
                self.problem.f64_state.insert(key, value);
            } else {
    ...
    fn metric(&mut self) {
        if self.token() == "(" {
            ...
            self.problem.metric = Some(match tag.as_str() {
                ...
                _ => panic!("Unrecognized metric tag: {}", tag)
            });
            self.check(")");
        }
    }
...
(:goal (on a b)))";
        let parsed = PddlParser::parse(pddl);
        assert_eq!(format!("{:?}", parsed), r#"PddlProblem { name: "blocks-2-0", domain: "blocks", obj2type: {"a": "untyped", "b": "untyped"}, bool_state: {Predicate { elements: ["clear", "a"] }, Predicate { elements: ["clear", "b"] }, Predicate { elements: ["handempty"] }, Predicate { elements: ["ontable", "a"] }, Predicate { elements: ["ontable", "b"] }}, f64_state: {}, goals: {Predicate { elements: ["on", "a", "b"] }}, metric: None }"#);
    }
...
(:goal (AND (ON A B) (ON B C) (ON C D)))
        ...
        let parsed = PddlParser::parse(pddl);
        assert_eq!(format!("{:?}", parsed), r#"PddlProblem { name: "blocks-4-2", domain: "blocks", obj2type: {"a": "untyped", "b": "untyped", "c": "untyped", "d": "untyped"}, bool_state: {Predicate { elements: ["clear", "a"] }, Predicate { elements: ["clear", "c"] }, Predicate { elements: ["clear", "d"] }, Predicate { elements: ["handempty"] }, Predicate { elements: ["on", "c", "b"] }, Predicate { elements: ["ontable", "a"] }, Predicate { elements: ["ontable", "b"] }, Predicate { elements: ["ontable", "d"] }}, f64_state: {}, goals: {Predicate { elements: ["on", "a", "b"] }, Predicate { elements: ["on", "b", "c"] }, Predicate { elements: ["on", "c", "d"] }}, metric: None }"#);
    }
